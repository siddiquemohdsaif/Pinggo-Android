...
final class JpegWebSocketTransport {
        ...
        webSocket = okHttpClient.newWebSocket(request, new WebSocketListener() {
            ...
            public void onClosed(WebSocket webSocket, int code, String reason) {
                Log.d(TAG, "websocket_closed | t=" + System.currentTimeMillis()
                        ...
                        + " code=" + code
                        + " reason=" + reason + " locallyClosed=" + closed
                        + " joined=" + joined + " peerConnected=" + peerConnected);
            ...
            public void onFailure(WebSocket webSocket, Throwable t, Response response) {
                Log.d(TAG, "websocket_failure | t=" + System.currentTimeMillis()
                        + " callId=" + callId + " userId=" + userId
                        + " locallyClosed=" + closed + " responseCode="
                        + (response == null ? "none" : response.code()), t);
                if (!closed && listener != null) {
                    listener.onError(t);
                }
            }
    ...
    void sendMediaState(boolean cameraEnabled, boolean microphoneMuted) {
        try {
            sendJson(new JSONObject().put("type", "client.media_state")
                    .put("callId", callId).put("userId", userId)
    ...
    void close() {
        Log.d(TAG, "websocket_close_requested | t=" + System.currentTimeMillis()
                + " callId=" + callId + " userId=" + userId
                + " hasSocket=" + (webSocket != null) + " closed=" + closed
                + " joined=" + joined + " peerConnected=" + peerConnected);
        if (webSocket != null) {
            sendLeave();
            closed = true;
        ...
        try {
            ...
            if ("server.joined".equals(type)) {
                ...
                if (remoteLeft && listener != null) {
                    Log.d(TAG, "disconnect_source | t=" + System.currentTimeMillis()
                            + " source=remote_participant_left callId=" + callId
                            + " participant=" + participant);
                    listener.onClosed();
                }
                ...
                if (!TextUtils.isEmpty(participantId) && !userId.equals(participantId)
                        && listener != null) {
                    listener.onRemoteCameraChanged(message.optBoolean("cameraEnabled", true));
                }
            } else if ("server.left".equals(type) && listener != null) {
                Log.d(TAG, "disconnect_source | t=" + System.currentTimeMillis()
                        + " source=server_left callId=" + callId + " message=" + text);
                listener.onClosed();
            } else if ("server.error".equals(type)) {
                String code = message.optString("code", "UNKNOWN");
                String errorMessage = message.optString("message",
                        "Server rejected JPEG transport request.");
                Log.w(TAG, "server_error | t=" + System.currentTimeMillis()
                        + " callId=" + callId + " code=" + code
                        + " message=" + errorMessage + " joined=" + joined
                        + " peerConnected=" + peerConnected);
                // A rejected media frame is recoverable. It must not tear down signaling/audio
                // or close an otherwise healthy media WebSocket.
                if (!code.startsWith("VIDEO_")) {
                    reportError(new IllegalStateException(errorMessage));
                }
            }
        } catch (JSONException error) {
            reportError(error);
