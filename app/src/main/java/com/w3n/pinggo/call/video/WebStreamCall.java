...
public final class WebStreamCall {
    ...
    void start() {
        ...
        transport = new JpegWebSocketTransport(
                ...
                new JpegWebSocketTransport.Listener() {
                    ...
                    @Override
                    public void onRemoteCameraChanged(boolean enabled) {
                        mainHandler.post(() -> {
    ...
    public void sendMediaState(boolean cameraEnabled, boolean microphoneMuted) {
        JpegWebSocketTransport activeTransport = transport;
        if (activeTransport != null && (state == State.CONNECTING || state == State.CONNECTED))
            activeTransport.sendMediaState(cameraEnabled, microphoneMuted);
    }
