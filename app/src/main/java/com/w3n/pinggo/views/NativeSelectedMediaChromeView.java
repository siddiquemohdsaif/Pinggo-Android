...
public final class NativeSelectedMediaChromeView extends View {
  ...
  private final Bitmap drawingAccent=color(0xFF22C56E);
  ...
  public void setMediaType(String type){fileMode="File".equals(type);videoMode="Video".equals(type);editorHeaderMode=!fileMode;if(fileMode)editorMode=EDITOR_NORMAL;build();}
  ...
  public void setDrawingColor(int value){drawingAccent.eraseColor(value);build();}
  ...
  private void buildFilePreview(float w,float h){
    ...
    chrome.add(button("file_sender",senderId,secondary,new RectF(px(22),footerTop+px(34),senderRight(w),footerTop+px(136)),id->{},13.6f,Color.WHITE,px(51)));
  ...
  private void buildImageEditor(float w,float h){
    ...
    if(editorMode==EDITOR_DRAW){
      ...
      chrome.add(button("draw_active","✎",drawingAccent,new RectF(w-px(145),topInset+px(18),w-px(25),topInset+px(138)),id->listener.onDraw(),30,Color.WHITE,px(60)));
      return;
    }
    float size=px(112),top=topInset+px(34);
    float[] centers=videoMode?new float[]{90,390,700,990}:new float[]{90,350,570,790,1010};
    String[] labels=videoMode?new String[]{"×","↓","Aa","✎"}:new String[]{"×","↓","↻","Aa","✎"};
    for(int i=0;i<labels.length;i++){
      float center=px(centers[i]);
      final int action=i;
      chrome.add(button("editor_"+i,labels[i],i==4?drawingAccent:secondary,new RectF(center-size/2,top,center+size/2,top+size),id->{
        if(action==0)listener.onBack();
        else if(action==1)listener.onDownload();
        else if(!videoMode&&action==2)listener.onRotate();
        else if((videoMode&&action==2)||(!videoMode&&action==3))listener.onText();
        else listener.onDraw();
      },"Aa".equals(labels[i])?22:30,Color.WHITE,size/2));
    }
    float footerTop=h-px(170);
    ...
    chrome.add(button("sender",senderId,secondary,new RectF(px(22),footerTop+px(34),senderRight(w),footerTop+px(136)),id->{},13.6f,Color.WHITE,px(51)));
  ...
  private float senderRight(float availableWidth){Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setTypeface(NativeFonts.load(getContext(),NativeFonts.INTER));paint.setTextSize(sp(13.6f));float contentWidth=paint.measureText(senderId)+px(64f);float maximumRight=availableWidth-px(175f);return Math.min(maximumRight,px(22f)+Math.max(px(150f),contentWidth));}
