...
// Enable only for short diagnostic captures. Production uses the existing periodic averages.
static constexpr bool kVerboseCallFrameLogs = false;
...
public:
    ...
    void start(JNIEnv* env, jobject source) {
        ...
        onLatestFilteredJpeg_ = env->GetMethodID(
                ...
                "([BIIJ)Lcom/w3n/pinggo/call/video/LatestFilteredFrameWorker$SendResult;");
...
private:
    ...
    void run() {
        ...
        while (running_) {
            ...
            const uint8_t* sendY = frame.yuv.data();
            const uint8_t* sendU = frame.yuv.data() + frame.yPlaneBytes;
            const uint8_t* sendV = frame.yuv.data() + frame.yPlaneBytes + frame.uvPlaneBytes;
            if (frame.width <= 0 || frame.height <= 0 || frame.yStride <= 0 || frame.uvStride <= 0) {
                LOGC("worker_encode_drop | t=%lld reason=invalid_filtered_yuv bufferVersion=%lld yuvBytes=%zu size=%dx%d yStride=%d uvStride=%d",
                     static_cast<long long>(wallTimeMs()),
            ...
            const bool encoded = nativeYuv420ToJpeg(sendY,
                                                    sendU,
                                                    sendV,
                                                    frame.width,
                                                    frame.height,
            ...
            if (encoded) {
                    if (kVerboseCallFrameLogs) LOGI("LatestFrameAvail | direct filtered jpeg=%zu bytes | quality=%d | %dx%d",
                         jpeg.size(),
                         jpegQualityPercent,
                         frame.width,
                         frame.height);
                    if (env && source_ && onLatestFilteredJpeg_) {
                        ...
                        if (kVerboseCallFrameLogs) LOGC("Send Frame %lld: camera to filtered yuv420 bufferVersion=%lld copyMs=%.3f yuvBytes=%zu, filtered yuv420 direct to jpeg quality=%d encodeMs=%.3f jpegBytes=%zu size=%dx%d yStride=%d uvStride=%d, jpeg callback callbackMs=%.3f, jpeg send websocket sent=%s queued=%s seq=%lld sendMs=%.3f totalMs=%.3f t=%lld",
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeStart(
        JNIEnv* env, jclass, jboolean front, jint width, jint height) {
    std::lock_guard<std::mutex> lock(gMutex);
    std::string result = camera()->start(front == JNI_TRUE, width, height);
    return env->NewStringUTF(result.c_str());
...
extern "C" JNIEXPORT void JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeStop(
        JNIEnv*, jclass) {
    std::lock_guard<std::mutex> lock(gMutex);
    if (gCamera) gCamera->stop();
}
...
extern "C" JNIEXPORT void JNICALL
Java_com_w3n_pinggo_call_video_LatestFilteredFrameWorker_nativeStart(
        JNIEnv* env, jclass, jobject source) {
    gLatestFilteredFrameWorker.start(env, source);
}

extern "C" JNIEXPORT void JNICALL
Java_com_w3n_pinggo_call_video_LatestFilteredFrameWorker_nativeStop(
        JNIEnv*, jclass) {
    gLatestFilteredFrameWorker.stop();
}

extern "C" JNIEXPORT void JNICALL
Java_com_w3n_pinggo_call_video_LatestFilteredFrameWorker_nativeShutdown(
        JNIEnv*, jclass) {
    gLatestFilteredFrameWorker.shutdown();
}

extern "C" JNIEXPORT void JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeSetMainPreviewRendering(
        JNIEnv*, jclass, jboolean enabled) {
    vulkanSetPreviewRenderingEnabled(enabled == JNI_TRUE);
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeSetSurface(
        JNIEnv* env, jclass, jobject surface, jobject assetManager) {
    if (!surface) {
        cameraPreprocessGpuDestroy();
        vulkanDestroy();
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeSetRemoteSurface(
        JNIEnv* env, jclass, jobject surface, jobject assetManager) {
    if (!surface) {
        remoteVulkanDestroy();
...
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeRenderJpegToMainSurface(
    ...
    if (kVerboseCallFrameLogs) LOGC("Received Frame %lld: javaByteArrayCopyMs=%.3f jpegBytes=%d, cpuJpegToYuv420DecodeMs=%.3f decodedYuv420Bytes=%zu yuvSize=%dx%d, remoteVulkanWallMs=%.3f rendered=%s rotation=%d mirror=%s totalReceiveRenderMs=%.3f t=%lld",
         ...
         static_cast<long long>(wallTimeMs()));
    if (rendered && kVerboseCallFrameLogs) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeSetDisplayRotation(
        JNIEnv*, jclass, jint rotationDegrees) {
    camera_transform_set_display_rotation(rotationDegrees);
}

extern "C" JNIEXPORT jintArray JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeGetPreviewTransform(
        JNIEnv* env, jclass) {
    CameraTransformation transform = camera_transform_evaluate();
    const jint values[2] = {
...
extern "C" JNIEXPORT jintArray JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeCaptureLatestFilteredArgb(
        JNIEnv* env, jclass, jintArray dimensions) {
...
extern "C" JNIEXPORT jobjectArray JNICALL
Java_com_w3n_pinggo_call_video_NativeVideoPipeline_nativeCaptureFilteredYuv420Ring(
        JNIEnv* env, jclass, jintArray info) {
